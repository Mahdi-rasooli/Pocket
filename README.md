...
frontend/
  src/
    ...
    components/
      ...
      ThemeToggle.tsx, LanguageSwitcher.tsx, CurrencySwitcher.tsx   Sidebar controls
      LocaleHtmlSync.tsx       Syncs <html lang/dir> to the active locale (client-side)
    lib/
      api.ts                 Fetch wrapper, attaches JWT from localStorage
      auth-context.tsx        React context for the logged-in user
      currency.ts / currency-context.tsx   Static USD conversion table + CurrencyProvider
      amount-preview.ts        Compact, locale-aware amount formatting (toman conversion for IRR)
      i18n/                    en.json / fa.json / fr.json dictionaries + I18nProvider (useI18n)
      types.ts                 TypeScript types mirroring backend response shapes
      format.ts                Category formatting + per-category chart colors
docker/
...
## Localization, theming & currency
- **Theme**: light/dark, toggled from the sidebar (`ThemeToggle.tsx`, backed by `next-themes`). All colors are CSS variables (`--background`, `--card`, `--primary`, `--chart-1..5`, etc., defined in `globals.css`) so shadcn components, charts, and the custom `bg-surface`/`.card` utilities all repaint automatically — no per-component light/dark branching needed.
- **Language**: English, Persian, French — switched from the sidebar (`LanguageSwitcher.tsx`). Translation strings live in `lib/i18n/{en,fa,fr}.json` as flat `"section.key": "..."` dictionaries, read via the `useI18n()` hook (`t('key')`). The choice is client-side only (stored in `localStorage`, no `/en /fa /fr` URL prefixes). When Persian is active, `LocaleHtmlSync.tsx` sets `<html dir="rtl" lang="fa">`, which flips layout direction app-wide via CSS logical properties.
  - **Known limitation**: the plain-language suggestion sentences on the Goals page are generated by the backend (`backend/src/services/suggestions.js`) in English and are not translated — that would require backend-side i18n, which is out of scope for now.
- **Dates**: `DatePicker.tsx` renders a Gregorian calendar (via shadcn's `Calendar` + `date-fns`) normally, and a Jalali/Shamsi calendar (via `date-fns-jalali`) when the locale is Persian. Either way the underlying value stored in form state and sent to the backend is a plain ISO `yyyy-MM-dd` string — the calendar system only affects rendering, so there's no schema or API impact.
- **Currency**: USD, EUR, IRR, JPY, switched from the sidebar (`CurrencySwitcher.tsx`). All amounts are still stored and computed in USD on the backend; `lib/currency.ts` holds a small static/approximate conversion table (**not live FX rates** — clearly commented as such) used only for display formatting and for converting typed amounts back to USD before they're sent to the API.
- **Live amount preview**: every amount field (`IncomeForm`, `ExpenseForm`, `RaiseForm`, `GoalForm`) shows a compact readout under the input as you type, e.g. "≈ 40K dollars" — via `AmountPreview.tsx` / `lib/amount-preview.ts`, using `Intl.NumberFormat(..., { notation: 'compact' })` in the active app locale. When the active currency is Iranian Rial, the preview converts to **toman** (1 toman = 10 rial — the everyday unit Iranians actually count in) instead of showing raw rial, e.g. "≈ ۴۰ میلیون تومان" for a 400,000,000-rial entry.
...
- **New expense category not showing correct color/label/translation**: category enums live in three places that must stay in sync — `backend/src/models/ExpenseEntry.js` (`EXPENSE_CATEGORIES`), `frontend/src/lib/types.ts` (`ExpenseCategory`), display colors in `frontend/src/lib/format.ts` (`CATEGORY_COLORS`), and a `category.<name>` key in each of `frontend/src/lib/i18n/{en,fa,fr}.json`.
- **Jalali dates look wrong / calendar doesn't switch**: confirm the active locale is actually `fa` (`useI18n().locale`) — `DatePicker.tsx` only swaps to `date-fns-jalali` for Persian; English and French always render the Gregorian calendar.
- **Currency numbers look off after switching**: the conversion table in `frontend/src/lib/currency.ts` is a static approximation, not a live feed — expect it to drift from real exchange rates over time. Update `rateToUSD` there if you need more current figures.
...
- Git history is incremental by design (one commit per completed feature/step) — see `git log` for the build order: backend scaffold → stats → goals/projections → dashboard → transactions → goals UI → Docker → shadcn/ui + theme → i18n (EN/FA/FR) → multi-currency → Jalali date picker → shadcn charts.
